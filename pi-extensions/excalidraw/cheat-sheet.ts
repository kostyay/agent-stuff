/**
 * Excalidraw element format cheat sheet for the LLM.
 *
 * Copied verbatim from https://github.com/excalidraw/excalidraw-mcp/blob/main/src/server.ts
 * (the RECALL_CHEAT_SHEET constant) with `create_view` renamed to `draw_diagram` to match
 * this extension tool name. Source license: MIT (Excalidraw).
 */

export const RECALL_CHEAT_SHEET = `# Excalidraw Element Format

Thanks for calling read_me! Do NOT call it again in this conversation — you will not see anything new. Now use draw_diagram to draw.
...
Every draw_diagram call returns a \`checkpointId\` in its response. To continue from a previous diagram state, start your elements array with a restoreCheckpoint element:
